const bundleBaseUrl = `${process.env.PUBLIC_URL}/bundle`

const getPaths = async () => {
  const indexRes = await fetch(`${bundleBaseUrl}/indexedfiles-v2.txt`)
...
const copyFiles = async (dirs, files, FS = window.FS) => {
  ...
  for (let i = 0; i < files.length; i++) {
    let fileRes = await fetch(`${bundleBaseUrl}/bundle/${files[i]}`)
